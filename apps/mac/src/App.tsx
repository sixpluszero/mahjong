...
const I18N: Record<Lang, Record<string, string>> = {
  en: {
    title: 'Mahjong Mac Client', subtitle: 'Lobby MVP', mode: 'Mode', preview: 'Preview', live: 'Live', language: 'Language', wsUrl: 'WS URL',
  ...
  zh: {
    title: '四川麻将 Mac 客户端', subtitle: '大厅 MVP', mode: '模式', preview: '预览', live: '在线', language: '语言', wsUrl: 'WS 地址',
...
export default function App(): JSX.Element {
  ...
  const [state, setState] = useState<LobbyViewState>({ name: '', roomId: '', players: [], connected: false, statusKey: 'idle' });
  ...
  const runtime = useMemo(() => createLobbyRuntime({
    ...
    onState: (patch) => setState((s) => ({
      ...s,
      ...patch,
      ...(patch.connected === false ? { players: [], roomId: '', roomPhase: '-', roundNo: 0, maxRounds: 0 } : {})
    }))
  }), [mode, wsUrl]);
