...
export type LobbyPlayer = { seat: number; name: string; isBot: boolean; ready: boolean; online: boolean; totalScore: number; melds: Meld[] };
...
function createLiveRuntime(wsUrl: string, onState: RuntimeOptions['onState']): LobbyRuntime {
  ...
  const buildPlayers = (roomPlayers: any[] = [], gamePlayers: any[] = []): LobbyPlayer[] => {
    ...
    return roomPlayers.filter((p: any) => p.occupied).map((p: any) => ({
      seat: p.seat, name: p.name, isBot: !!p.isBot, ready: !!p.ready, online: p.online !== false, totalScore: p.totalScore ?? 0, melds: meldBySeat.get(p.seat) || []
    }));
  };
